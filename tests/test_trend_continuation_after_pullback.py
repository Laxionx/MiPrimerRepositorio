...
def _bar(index: int, *, close: float, high: float | None = None, low: float | None = None, symbol: str = "XAUUSD", timeframe: str = "M5") -> dict:
    return {
        "timestamp_utc": datetime(2026, 1, 1, tzinfo=UTC) + timedelta(minutes=5 * index),
        "symbol": symbol, "timeframe": timeframe, "open_bid": close - 0.03,
        "high_bid": close + 0.10 if high is None else high,
        "low_bid": close - 0.10 if low is None else low,
        "close_bid": close, "spread_points": 10.0, "point_size": 0.01,
        "is_completed": True,
    }
...
def _long_pullback_fixture() -> list[dict]:
    ...
    rows.append(_bar(70, close=106.45, high=106.55, low=106.00))
    rows.append(_bar(71, close=107.15, high=107.25, low=106.70))
    rows.extend(_bar(index, close=107.15 + 0.1 * (index - 71)) for index in range(72, 83))
    return rows


def _short_pullback_fixture() -> list[dict]:
    rows = [_bar(index, close=120.0 - 0.10 * index, symbol="EURUSD") for index in range(70)]
    rows.append(_bar(70, close=113.55, high=114.00, low=113.45, symbol="EURUSD"))
    rows.append(_bar(71, close=112.85, high=113.30, low=112.75, symbol="EURUSD"))
    rows.extend(_bar(index, close=112.85 - 0.1 * (index - 71), symbol="EURUSD") for index in range(72, 83))
    return rows
...
@pytest.mark.parametrize("fixture,direction", [(_long_pullback_fixture, "long"), (_short_pullback_fixture, "short")])
...
def test_excessive_pullback_invalidation_duration_cooldown_and_rearm():
    ...
    rows = _long_pullback_fixture() + _long_pullback_fixture()[70:72]
    for index, row in enumerate(rows):
        row["timestamp_utc"] = datetime(2026, 1, 1, tzinfo=UTC) + timedelta(minutes=5 * index)
    assert len(tcp.generate_events(rows)["events"]) == 1
...
def test_missing_gap_duplicate_and_overlap_are_fail_closed_and_deterministic():
    ...
    assert all(event["terminal_category"] == "suppressed_overlap" for event in result["events"])
